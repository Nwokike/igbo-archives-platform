...
from archives.models import Archive, ArchiveItem, ArchiveNote, Author, Category
            ...
            try:
                ...
                if content_length and int(content_length) > self.max_download_size:
                    raise serializers.ValidationError(
                        f"File at URL exceeds the {self.max_download_size // (1024 * 1024)}MB limit."
                    )
                ...
                raise serializers.ValidationError(
                    f"Could not fetch media from provided AI URL. Ensure the URL is public and valid. Underlying error: {e}"
                )
...
class ArchiveNoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ArchiveNote
        fields = ["id", "archive_id", "content", "added_by", "is_approved", "created_at", "updated_at"]


class ArchiveNoteCreateSerializer(serializers.ModelSerializer):
    """Write-serializer for creating/appending notes to an archive."""

    archive_id = serializers.PrimaryKeyRelatedField(queryset=Archive.objects.all(), source="archive")
...
class LorePostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LorePost
        fields = ["id", "title", "slug", "content", "excerpt", "featured_image", "author", "created_at"]
