...
class ChatService:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.router = Router(model_list=config.get("model_list", []), **(config.get("router_settings", {})))
    ...
    def chat(self, messages: list, use_web_search: bool = True) -> dict:
        ...
        if use_web_search and user_message:
            ...
            if (
                any(t in user_message.lower() for t in ["igbo", "ala", "nri", "culture"])
                and "igbo" not in user_message.lower()
            ):
                search_query += " Igbo"
    ...
    def _litellm_chat(self, messages: list, context: str, model_name: str) -> dict:
        ...
        try:
            ...
            response = self.router.completion(
                model=model_name, messages=full_messages, max_tokens=self.max_tokens, temperature=self.temperature
            )
