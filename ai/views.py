...
def generate_tts(request):
    ...
    if result["success"]:
        ...
        cache_data = {"bytes": result["audio_bytes"], "content_type": result["content_type"]}
        ...
        return JsonResponse({"success": True, "url": serve_url, "provider": result["provider"]})

    return JsonResponse({"success": False, "error": result.get("error", "TTS failed")})
...
def generate_lore_content(request):
    ...
    try:
        ...
        if response["success"]:
            ...
            return JsonResponse({"success": True, "content": sanitized_content})
        else:
            return JsonResponse({"success": False, "error": response.get("content", "AI generation failed")})
