...
class AIViewTests(TestCase):
    ...
    def test_chat_send_stateless(self):
        ...
        with patch("ai.views.chat_service.chat") as mocked_chat:
            ...
            response = self.client.post(
                reverse("ai:chat_send"), data=json.dumps({"message": "Hello AI"}), content_type="application/json"
            )
    ...
    def test_chat_send_with_history(self):
        ...
        with patch("ai.views.chat_service.chat") as mocked_chat:
            ...
            response = self.client.post(
                ...
                data=json.dumps({"message": "How is it going?", "history": history}),
    ...
    def test_analyze_archive_stateless(self):
        ...
        archive = Archive.objects.create(
            title="Test Item", is_approved=True, uploaded_by=self.user, image=uploaded_image
        )
...
class RouterIntegrationTests(TestCase):
    ...
    def test_chat_calls_primary_model(self, mock_completion):
        ...
        mock_completion.return_value = MagicMock(
            choices=[MagicMock(message=MagicMock(content="Hello!"))], model="kimi-k2"
        )
    ...
    def test_vision_calls_primary_model(self, mock_completion):
        ...
        mock_completion.return_value = MagicMock(
            choices=[MagicMock(message=MagicMock(content="I see a cat."))], model="gemma-4-31b-it"
        )

        # Mock a small file for analysis
        with (
            patch("ai.services.vision_service.Path.exists", return_value=True),
