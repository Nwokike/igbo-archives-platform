...
from ai.models import ArchiveAnalysis, ChatMessage
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Chat usage
        chat_messages = ChatMessage.objects.filter(created_at__gte=cutoff, role="assistant")
        ...
        # Model breakdown
        model_usage = chat_messages.values("model_used").annotate(count=Count("id")).order_by("-count")
        ...
        type_usage = analyses.values("analysis_type").annotate(count=Count("id")).order_by("-count")
