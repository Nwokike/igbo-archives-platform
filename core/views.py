...
def get_all_approved_archive_ids():
    ...
    if archive_ids is None:
        archive_ids = tuple(
            Archive.objects.filter(is_approved=True).values_list("id", flat=True).iterator(chunk_size=1000)
        )
...
def get_random_featured_archives(max_count=50):
    ...
    if image_ids is None:
        ...
        image_ids = list(Archive.objects.filter(is_approved=True, archive_type="image").values_list("id", flat=True))
    ...
    archives = list(Archive.objects.filter(pk__in=selected_ids).select_related("category", "uploaded_by"))
...
def home(request):
    ...
    context = {"featured_archives": featured_archives}
