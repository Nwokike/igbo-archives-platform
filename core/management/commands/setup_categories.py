...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for name in archive_cats:
            slug = slugify(name)
            cat, created = Category.objects.get_or_create(slug=slug, defaults={"name": name, "type": "archive"})
        ...
        for name in lore_cats:
            slug = slugify(name)
            cat, created = Category.objects.get_or_create(slug=slug, defaults={"name": name, "type": "lore"})
