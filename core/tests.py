    ...
    def test_profile_picture_update_cleans_old_media(self):
        ...
        with patch(f"{backend}.delete") as mock_delete:
            ...
            user.profile_picture = SimpleUploadedFile(
                name="old_pfp.jpg", content=b"old_content", content_type="image/jpeg"
            )
            user.save()
            ...
            with self.captureOnCommitCallbacks(execute=True):
                user.profile_picture = SimpleUploadedFile(
                    name="new_pfp.jpg", content=b"new_content", content_type="image/jpeg"
                )
                user.save()
    ...
    def test_bulk_deletion_cleans_media(self):
        ...
        with patch(f"{backend}.delete") as mock_delete:
            ...
            a1 = Archive.objects.create(
                ...
                description="D",
                archive_type="image",
                image=SimpleUploadedFile("a1.jpg", b"c"),
                uploaded_by=self.user,
            )
            a2 = Archive.objects.create(
                ...
                description="D",
                archive_type="image",
                image=SimpleUploadedFile("a2.jpg", b"c"),
                uploaded_by=self.user,
            )
...
class BackgroundTaskTests(TestCase):
    ...
    def test_send_email_async(self, mock_send_mail):
        ...
        result = send_email_async.func(
            subject="Test Subject", message="Test Message", recipient_list=["test@example.com"]
        )
    ...
    def test_send_push_notification_async(self, mock_send_push):
        ...
        # Test calling directly using .func to bypass async queue
        result = send_push_notification_async.func(user_id=user.id, title="Test Push", body="Test Body", url="/test/")
