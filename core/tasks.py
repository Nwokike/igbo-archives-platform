...
def cleanup_old_notifications():
    ...
    try:
        ...
        deleted_count, _ = Notification.objects.filter(timestamp__lt=cutoff).delete()
...
def cleanup_old_messages():
    ...
    try:
        ...
        old_threads = (
            Thread.objects.filter(updated_at__lt=cutoff)
...
def cleanup_old_system_logs():
    ...
    try:
        ...
        deleted_digests, _ = DigestQueue.objects.filter(processed=True, processed_at__lt=cutoff).delete()
...
def cleanup_deactivated_accounts():
    ...
    try:
        ...
        deactivated = User.objects.filter(is_active=False, deactivated_at__isnull=False, deactivated_at__lt=cutoff)
...
def deactivate_idle_accounts():
    ...
    try:
        ...
        warn_users = User.objects.filter(
            is_active=True, last_login__lt=warn_cutoff, last_login__gte=deactivate_cutoff, deactivated_at__isnull=True
        ...
        idle_users = User.objects.filter(is_active=True, last_login__lt=deactivate_cutoff).exclude(is_staff=True)
...
def _reassign_user_content_to_admin(users_qs):
    ...
    try:
        ...
        if total:
            logger.info(
                f"Content reassigned to admin: {archives_count} archives, {lore_count} lore, {books_count} books, {notes_count} notes"
            )
    except Exception as e:
...
def send_weekly_digest():
    ...
    try:
        ...
        eligible_users = (
            ...
            .filter(Q(last_weekly_update_at__lt=last_week) | Q(last_weekly_update_at__isnull=True))
            .exclude(email="")
        )
        ...
        recent_notes = ArchiveNote.objects.filter(
            is_approved=True, created_at__gte=now - timedelta(days=7)
        ).select_related("archive", "added_by")[:10]

        # Build notes list from both queued items and direct query
        notes_list = [
            {
                "title": n.archive.title,
                "author_name": n.added_by.get_display_name() if n.added_by else "Community",
                "url": n.archive.get_absolute_url(),
            }
        ...
        context = {
            ...
            "year": week_end.year,
            "new_archives": [
                {"title": i.title, "author_name": i.author_name, "url": i.url, "archive_type": "Archive"}
                for i in archives[:10]
            ],
