...
ALLOWED_TAGS = ["a", "b", "strong", "i", "em", "u", "br", "span", "mark", "code", "sub", "sup"]
...
def render_quote(data):
    ...
    if caption:
        html += (
            f'<cite class="block mt-2 text-sm text-text-muted dark:text-text-dark-muted not-italic">— {caption}</cite>'
        )
    html += "</blockquote>"
    return html
