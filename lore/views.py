...
def lore_list(request):
    ...
    if search := request.GET.get("search"):
        posts = posts.filter(
            Q(title__icontains=search) | Q(excerpt__icontains=search) | Q(legacy_content__icontains=search)
        )
    ...
    from core.validators import ALLOWED_LORE_SORTS, get_safe_sort
    ...
    context = {"posts": page_obj, "categories": get_cached_lore_categories()}
...
def lore_detail(request, slug):
    ...
    previous_post = (
        LorePost.objects.filter(published_filter, created_at__lt=post.created_at)
        .order_by("-created_at")
        .only("id", "title", "slug", "featured_image", "image_url")
        .first()
    )

    next_post = (
        LorePost.objects.filter(published_filter, created_at__gt=post.created_at)
        .order_by("created_at")
        .only("id", "title", "slug", "featured_image", "image_url")
        .first()
    )
    ...
    similar_ids = list(
        LorePost.objects.filter(category=post.category, is_approved=True, is_published=True)
        .exclude(id=post.id)
...
def lore_create(request):
    ...
    context = {"form": form, "title": "Share Cultural Lore", "submit_text": "Submit Post"}
