...
class LorePostAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("title", "slug", "author", "original_author", "category", "excerpt")}),
        (
            "Media",
            {
                "fields": (
        ...
        ("Status", {"fields": ("is_published", "is_approved", "pending_approval", "is_rejected", "rejection_reason")}),
    ...
    def reject_posts(self, request, queryset):
        ...
        for post in queryset.filter(is_rejected=False):
            ...
            send_post_rejected_notification(
                post, reason=post.rejection_reason or "Did not meet guidelines", post_type="lore"
            )
            count += 1
