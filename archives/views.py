...
from .models import Archive, Author, Category
...
def archive_list(request):
    ...
    archives = (
        ...
        .select_related("uploaded_by", "category", "author")
        .only(
            "id",
            "title",
            "archive_type",
    ...
    if search := request.GET.get("search"):
        archives = archives.filter(
            Q(title__icontains=search) | Q(description__icontains=search) | Q(original_author__icontains=search)
        )
...
def archive_detail(request, pk=None, slug=None):
    ...
    if slug:
        archive = get_object_or_404(
            Archive.objects.select_related("uploaded_by", "category", "author").prefetch_related("items"), slug=slug
        )
    elif pk:
        archive = get_object_or_404(
            Archive.objects.select_related("uploaded_by", "category", "author").prefetch_related("items"), pk=pk
        )
        if archive.slug:
    ...
    similar_archives = (
        Archive.objects.filter(is_approved=True, category=archive.category)
        .exclude(pk=archive.pk)
        .select_related("uploaded_by", "author")[:5]
    )

    previous_archive = (
        Archive.objects.filter(is_approved=True, created_at__lt=archive.created_at)
        .order_by("-created_at")
        .only("id", "title", "slug")
        .first()
    )
    ...
    next_archive = (
        Archive.objects.filter(is_approved=True, created_at__gt=archive.created_at)
        .order_by("created_at")
        .only("id", "title", "slug")
        .first()
    )
    ...
    if not ai_correlations and archive.is_approved:
        ...
        recommended_books = get_similar_items(
            target_text, books_qs, limit=9, text_field=lambda b: f"{b.book_title} {b.author} {b.title} {b.content_json}"
        )

        lores_qs = LorePost.objects.filter(is_approved=True)
        recommended_lores = get_similar_items(
            target_text, lores_qs, limit=9, text_field=lambda l: f"{l.title} {l.excerpt} {l.content_json}"
        )
...
def author_detail(request, slug):
    ...
    archives_list = (
        Archive.objects.filter(is_approved=True, author=author)
    ...
    lore_posts = (
        LorePost.objects.filter(is_published=True, is_approved=True, original_author__iexact=author.name)
        .select_related("category", "author")
        .order_by("-created_at")
    )

    recommended_books = (
        BookRecommendation.objects.filter(is_published=True, is_approved=True, author__iexact=author.name)
...
def metadata_suggestions(request):
    ...
    if len(query) > 1:
        if field == "author":
            results = list(Author.objects.filter(name__icontains=query).values_list("name", flat=True)[:10])

            if len(results) < 5:
                text_results = (
                    Archive.objects.filter(original_author__icontains=query)
                    .values_list("original_author", flat=True)
                    .distinct()[:5]
                )
            ...
            results = list(
                Archive.objects.filter(circa_date__icontains=query).values_list("circa_date", flat=True).distinct()[:10]
            )
...
def archive_create(request):
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                if not items:
                    ...
                    return render(
                        request,
                        "archives/create.html",
                        {"form": form, "formset": formset, "categories": get_cached_categories()},
                    )
    ...
    context = {"form": form, "formset": formset, "categories": get_cached_categories()}
...
def add_archive_note(request, slug):
    ...
    if request.method == "POST":
        ...
        note = ArchiveNote.objects.create(
            archive=archive, added_by=request.user, content_json=parsed_json, is_approved=False
        )
...
def suggest_note_edit(request, note_id):
    ...
    if request.method == "POST":
        ...
        suggestion = ArchiveNoteSuggestion.objects.create(
            note=note, suggested_by=request.user, suggestion_text=parsed_json
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not author.description:
                ...
                author.save()
                messages.success(
                    request, "Thank you! Your description and photo have been automatically approved and updated."
                )
            else:
                req.save()
...
def author_suggestions(request):
    ...
    lores = (
        LorePost.objects.filter(original_author__icontains=query)
        .values_list("original_author", flat=True)
        .distinct()[:10]
    )
