...
from .models import Archive, ArchiveItem, Author, Category
...
def reject_archives(modeladmin, request, queryset):
    ...
    for archive in queryset.filter(is_rejected=False):
        ...
        send_post_rejected_notification(
            archive, reason=archive.rejection_reason or "Did not meet guidelines", post_type="archive"
        )
        count += 1
        count += 1
...
class ArchiveAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Info", {"fields": ("title", "slug", "description", "archive_type", "category", "item_count")}),
        ("Primary Media (Cover)", {"fields": ("image", "video", "audio", "document", "featured_image")}),
        ("Caption & Copyright", {"fields": ("caption", "copyright_holder", "alt_text")}),
        ("Source Information", {"fields": ("original_author", "author", "original_url", "original_identity_number")}),
        ("Date & Location", {"fields": ("date_created", "circa_date", "location", "sort_year")}),
        (
            "Status",
